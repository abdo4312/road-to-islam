...
import { Check, X, User, MessageCircle, Clock, ChevronLeft } from 'lucide-react';
...
export const MentorDashboard = ({ goBack }: { goBack: () => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col h-full bg-bg-light dark:bg-bg-dark pb-28 overflow-y-auto">
      {/* Header */}
      <div className="bg-primary text-white p-6 pt-12 pb-6 rounded-b-3xl shadow-lg sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <button onClick={goBack} className="p-1.5 bg-white/10 hover:bg-white/20 transition rounded-full backdrop-blur-sm border border-white/20">
            <ChevronLeft size={24} />
          </button>
          <div>
            <h2 className="text-2xl font-bold font-serif text-accent">{t('mentorDashboard.title')}</h2>
            <p className="text-emerald-100/80 text-sm">{t('mentorDashboard.subtitle')}</p>
          </div>
        </div>
      </div>
            ...
            <div key={req.id} className="bg-white dark:bg-[#1a2e1e] p-5 rounded-2xl shadow-sm border border-primary/10">
              ...
              {req.status === 'pending' && (
                <div className="space-y-2">
                  ...
                  {/* WhatsApp direct reply - available immediately without approving */}
                  {req.student_whatsapp && (
                    <button
                      ...
                      className="w-full py-2 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366]
                        rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition"
                    >
              ...
              {req.status === 'approved' && (
                <div className="space-y-2">
                  <button
                    ...
                    className="w-full py-2.5 bg-primary/10 hover:bg-primary/20 dark:bg-accent/10 dark:hover:bg-accent/20
                      text-primary dark:text-accent rounded-xl font-bold text-sm
